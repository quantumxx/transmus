...
def transfer_youtube_to_spotify(
    ...
    # Deduplicate matched tracks to avoid adding the same song twice
    matched_pairs = _deduplicate_matched_pairs(matched_pairs, key=lambda t: t.source_uri)

    if progress_callback:
        progress_callback(
            f"Matched {len(matched_pairs)} unique tracks after deduplication",
            75,
        )
...
def transfer_spotify_to_youtube(
    ...
    # Deduplicate matched tracks to avoid adding the same song twice
    matched_pairs = _deduplicate_matched_pairs(matched_pairs, key=lambda t: t.source_id)
...
def transfer_youtube_liked_to_spotify(
    new_name: Optional[str] = None,
    public: bool = True,
    progress_callback: Optional[Callable] = None,
) -> TransferResult:
    """Transfer YouTube Music Liked Songs to a new Spotify playlist.

    Args:
        new_name: Optional name for the new Spotify playlist.
                  Defaults to "YouTube Music Liked Songs".
        public: Whether the new Spotify playlist should be public.
        progress_callback: Optional callback for progress updates.
    ...
    # Step 1: Fetch liked songs from YouTube Music
    if progress_callback:
        progress_callback("Fetching YouTube Music Liked Songs...", 0)

    source_playlist = yt.get_liked_tracks()
    dest_name = new_name or "YouTube Music Liked Songs"

    if progress_callback:
        progress_callback(
            f"Found {len(source_playlist.tracks)} liked songs",
            10,
        )
    ...
    dest_id = sp.create_playlist(
        name=dest_name,
        description="Transferred from YouTube Music Liked Songs",
        public=public,
        track_uris=track_uris,
    )
...
def transfer_spotify_liked_to_youtube(
    new_name: Optional[str] = None,
    public: bool = True,
    progress_callback: Optional[Callable] = None,
) -> TransferResult:
    """Transfer Spotify Liked Songs to a new YouTube Music playlist.

    Args:
        new_name: Optional name for the new YouTube Music playlist.
                  Defaults to "Spotify Liked Songs".
        public: Whether the new YouTube Music playlist should be public.
        progress_callback: Optional callback for progress updates.
    ...
    if progress_callback:
        progress_callback("Fetching Spotify Liked Songs...", 0)

    source_playlist = sp.get_liked_tracks()
    dest_name = new_name or "Spotify Liked Songs"
    ...
    dest_id = yt.create_playlist(
        name=dest_name,
        description="Transferred from Spotify Liked Songs",
        track_ids=track_ids,
    )
...
def _deduplicate_matched_pairs(
    matched_pairs: list[tuple[Track, Track]],
    key,
) -> list[tuple[Track, Track]]:
    """Remove duplicate destination tracks from matched pairs.

    Keeps the first occurrence of each unique destination track,
    dropping subsequent duplicates. Uses the provided key function
    to extract a unique identifier from the destination track.

    Args:
        matched_pairs: List of (source_track, dest_track) pairs.
        key: Function that takes a Track and returns a hashable identifier.

    Returns:
        Deduplicated list of (source_track, dest_track) pairs.
    """
    seen: set = set()
    deduped: list[tuple[Track, Track]] = []
    for source_track, dest_track in matched_pairs:
        identifier = key(dest_track)
        if identifier and identifier not in seen:
            seen.add(identifier)
            deduped.append((source_track, dest_track))
    return deduped